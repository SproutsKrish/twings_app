...
/* Start Inserting deviceimei Current Details Get From Livedata Table */
 SELECT deviceimei,client_id,vehicle_id,vehicle_current_status,vehicle_status,odometer,ignition_report_flag,ignition_report_datetime,device_battery_volt,vehicle_battery_volt,lattitute,longitute,device_updatedtime,TIMESTAMPDIFF(
    MINUTE,
    ignition_report_datetime,
    NEW.device_datetime
  ),current_alert_status
            INTO @deviceimei,@client_id,@vehicle_id,@vehicle_current_status,@vehicle_status,@odometer,@ignition_report_flag,@ignition_report_datetime,@device_battery_volt,@vehicle_battery_volt,@lattitute,@longitute,@device_updatedtime,@diff_time,@current_alert_status
            FROM live_data
            WHERE
...
SELECT parking_alert_time,idle_alert_time,speed_limit INTO @parking_alert_time,@idle_alert_time,@speed_limit FROM twings.configurations WHERE client_id=@client_id AND device_imei = @device_imei ORDER BY device_imei DESC LIMIT 1;
...
END IF;
IF (@current_alert_status!=21 OR @current_alert_status=0) AND current_vehicle_status=1 AND @diff_time>@idle_alert_time THEN
INSERT INTO twings.live_notifications(`vehicle_id`, `device_imei`, `alert_type_id`, `lattitute`, `longitute`, `ignition`, `speed`, `angle`, `odometer`, `device_updatedtime`, `ignition_flag`, `user_id`, `created_at`, `updated_at`) VALUES (@vehicle_id,@deviceimei,20,NEW.lattitute,NEW.longitute,NEW.ignition,NEW.speed,NEW.angle,current_odometer,NEW.device_datetime,0,@client_id,now(),now());
UPDATE live_data SET current_alert_status=21 WHERE deviceimei = NEW.deviceimei;
END IF;
IF (@current_alert_status!=5 OR @current_alert_status=0) AND current_vehicle_status=1 AND @diff_time>@speed_limit THEN
INSERT INTO twings.live_notifications(`vehicle_id`, `device_imei`, `alert_type_id`, `lattitute`, `longitute`, `ignition`, `speed`, `angle`, `odometer`, `device_updatedtime`, `ignition_flag`, `user_id`, `created_at`, `updated_at`) VALUES (@vehicle_id,@deviceimei,20,NEW.lattitute,NEW.longitute,NEW.ignition,NEW.speed,NEW.angle,current_odometer,NEW.device_datetime,0,@client_id,now(),now());
UPDATE live_data SET current_alert_status=5 WHERE deviceimei = NEW.deviceimei;
/* ADD OVERSPEED REPORT HERE */
END IF;

IF (@current_alert_status!=1 OR @current_alert_status=0) AND NEW.ac_status=1 AND (NEW.io_state=0 OR NEW.io_state=4) THEN
...
UPDATE live_data SET ac_status=1,current_alert_status=1 WHERE deviceimei = NEW.deviceimei;
...
END IF;
IF (@current_alert_status!=2 OR @current_alert_status=0) AND NEW.ac_status=0 AND (NEW.io_state=0 OR NEW.io_state=4) THEN
...
UPDATE live_data SET ac_status=0,current_alert_status=2 WHERE deviceimei = NEW.deviceimei;
