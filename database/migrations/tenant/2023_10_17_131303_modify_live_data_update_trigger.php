...
return new class extends Migration
{
 ...
 BEGIN
  -- Declare variables
DECLARE circle_radius INT; -- Set your desired radius here
DECLARE f_lattitude DOUBLE;
DECLARE f_longitude DOUBLE;
DECLARE earthRadius DOUBLE DEFAULT 6371; -- Earth's radius in kilometers
DECLARE deltaLat DOUBLE;
DECLARE deltaLon DOUBLE;
DECLARE a DOUBLE;
DECLARE c DOUBLE;
DECLARE lat1 DOUBLE;
DECLARE lon1 DOUBLE;
 ...
 /* START - CALCULATION OF ODOMETER */
            ...
            IF NEW.distance_with_odometer != '0' AND NEW.distance_with_odometer != '0.0' THEN
                             ...
                             ELSEIF NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND (NEW.io_state=0 OR NEW.io_state= 2 OR NEW.io_state=4) AND NEW.packet_status=0 AND NEW.ignition=1 AND NEW.speed>1 THEN
                                 SET lat1 = RADIANS(@lattitute);
  SET lon1 = RADIANS(@longitute);
  SET lat2 = RADIANS(NEW.lattitute);
  SET lon2 = RADIANS(NEW.longitute);
                                 -- Haversine formula
  SET deltaLat = lat2 - lat1;
  SET deltaLon = lon2 - lon1;
  SET a = SIN(deltaLat / 2) * SIN(deltaLat / 2) + COS(lat1) * COS(lat2) * SIN(deltaLon / 2) * SIN(deltaLon / 2);
  SET c = 2 * ATAN2(SQRT(a), SQRT(1 - a));
  SET current_distance = earthRadius * c;

                                  IF current_distance != 0 AND current_distance IS NOT NULL THEN
                                     INSERT INTO `test`(`device_imei`,`lat1`,`long1`,`lat2`,`long2`,`distance`, `device_time`) VALUES (NEW.deviceimei,@lattitute,@longitute,NEW.lattitute,NEW.longitute,current_distance,now());
                                     SET current_odometer = @odometer + current_distance;
                                 END IF;
                             END IF;
            END IF;
...
IF NEW.io_state=3 AND NEW.ignition=0 THEN
 UPDATE live_data
 SET
 ignition = NEW.ignition,
 speed = NEW.speed,
 ...
 WHERE deviceimei = @deviceimei;
 END IF;
  IF NEW.io_state=3 AND NEW.ignition=1 THEN
 UPDATE live_data
 SET
 ignition = NEW.ignition,
 speed=0,
...
IF((@ignition_report_flag = 1 OR @ignition_report_flag = 0 OR @ignition_report_flag IS NULL) AND (NEW.ignition = 1) AND NEW.packet_status = 0 ) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND (NEW.io_state!=1) THEN
/* LIVE DATA UPDATING */
UPDATE live_data
SET
...
IF(ROUND(NEW.speed) = 0 AND @ignition_report_flag = 2 AND NEW.ignition = 1 AND NEW.packet_status = 0 ) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND (NEW.io_state=0 OR NEW.io_state=2 OR NEW.io_state=4)  THEN
...
IF NEW.packet_status=0 AND NEW.io_state!=1 THEN
...
END IF;
 IF (@current_alert_status!=4 OR @current_alert_status=0) AND NEW.ignition=0 AND NEW.io_state!=1 THEN
