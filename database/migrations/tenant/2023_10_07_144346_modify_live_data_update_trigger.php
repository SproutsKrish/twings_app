...
SELECT ignition_flag INTO @current_ignition FROM twings.live_notifications WHERE device_imei=NEW.deviceimei ORDER BY id desc LIMIT 1;
 ...
 IF NEW.packet_status = 0 AND (NEW.io_state=2) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND NEW.ignition=1 THEN
  UPDATE live_data
  SET
  ignition = NEW.ignition,
 device_updatedtime = NEW.device_datetime,
 ...
 power_status = NEW.power_status
  WHERE deviceimei = @deviceimei;
  END IF;
   IF NEW.packet_status = 0 AND (NEW.io_state=2) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND NEW.ignition=0 THEN
  UPDATE live_data
  SET
  ignition = NEW.ignition,
 device_updatedtime = NEW.device_datetime,
 ...
 IF((@ignition_report_flag = 1 OR @ignition_report_flag = 0 OR @ignition_report_flag IS NULL) AND (NEW.ignition = 1) AND NEW.packet_status = 0 ) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND (NEW.io_state=0 OR NEW.io_state=2 OR NEW.io_state=4) THEN
 /* LIVE DATA UPDATING */
 UPDATE live_data
 SET
 ...
 IF(ROUND(NEW.speed) = 0 AND @ignition_report_flag = 2 AND NEW.ignition = 1 AND NEW.packet_status = 0 ) AND NEW.lattitute!= '0.000000' AND NEW.longitute!= '0.000000' AND NEW.io_state!=1  THEN
 ...
 IF NEW.packet_status=0 AND NEW.io_state!=1 THEN
 	IF NEW.io_state=0 OR NEW.io_state=4 THEN
    ...
    IF NEW.io_state=2 AND NEW.ignition=0  THEN
    SET current_vehicle_status=1;
    END IF;
    IF NEW.io_state=2 AND NEW.ignition=1 AND NEW.speed>1 THEN
    SET current_vehicle_status=3;
    END IF;
    IF NEW.io_state=2 AND NEW.ignition=1 AND NEW.speed=0 THEN
    SET current_vehicle_status=2;
    END IF;
 ...
 IF (@current_alert_status!=20 OR @current_alert_status=0) AND current_vehicle_status=1 AND @diff_time>30 THEN
 ...
 END IF;
 IF (@current_alert_status!=21 OR @current_alert_status=0) AND current_vehicle_status= 2 AND @diff_time>30 THEN
