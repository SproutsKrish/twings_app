...
class GeofenceReportController extends Controller
{
    public function geofence_report(Request $request)
    {
        $geofence_reports = GeofenceReport::select(
            ...
            ->join('assign_geofences as B', 'A.assign_georeport_id', '=', 'B.id')
            ...
            ->whereRaw("DATE_ADD(A.out_datetime, INTERVAL 330 MINUTE) >= ?", [$request->input('start_day')])
            ->whereRaw("DATE_ADD(A.in_datetime, INTERVAL 330 MINUTE) <= ?", [$request->input('end_day')])
            ->get();
